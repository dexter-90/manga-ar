...
class MangaDL:
    ...
    def __init__(self, name: str = None, url: str = None, start: int = None, end: int = None):
        ...
        elif url is not None and "https" not in url and '3asq.org' not in url:
            raise Exception(
    ...
    def DownloadChapters(self):
        ...
        a = chapters[self.start - 1:self.end + 1]
        if not a:
            if self.name is not None:
                Utility.Chapters(name=Utility.Search2(name=self.name))
        elif not a:
...
class Utility:
    ...
    def Search2(self, name: str):
        ...
        soup = BeautifulSoup(response.text, 'html.parser').find('h3', {'class': "h4"}).find('a')['href']
        return soup
